...
class MUNIT_Trainer(nn.Module):
    ...
    def compute_vgg_loss(self, vgg, img, target):
        ...
        # 如果 vgg() 返回的是 tuple，则取第一个 feature map
        if isinstance(img_fea, tuple):
            img_fea = img_fea[0]
        if isinstance(target_fea, tuple):
            target_fea = target_fea[0]

        img_fea_norm = self.instancenorm(img_fea)
        target_fea_norm = self.instancenorm(target_fea)

        return torch.mean((img_fea_norm - target_fea_norm) ** 2)
