...
def get_config(config):
    with open(config, 'r') as stream:
        return yaml.load(stream, Loader=yaml.FullLoader)  # 添加 Loader 参数
...
def load_vgg16(model_dir):
    ...
    if not os.path.exists(weight_path):
        ...
        torch.save(vgg16.state_dict(), weight_path)
        print(f"✅ VGG16 预训练模型已下载并保存至 {weight_path}")

    # 加载模型
    vgg = Vgg16()
    vgg.load_state_dict(torch.load(weight_path, map_location=torch.device('cpu')), strict=False)
    return vgg
